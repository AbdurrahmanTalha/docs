...
import CodeWithEnv from '@site/src/components/CodeWithEnv';
import { Button } from '@site/src/components/ui/button';
import {
  AccordionWrapper,
  AccordionItem,
} from '@site/src/components/Accordion';
import EnvValue from '@site/src/components/EnvValue';
...
<Tabs>
  <TabItem value="curl" label="Curl">
    <CodeWithEnv>
      ...
      curl --request GET \
        --url '<EnvValue name="dashboardAPI"/>/scraper?api_key=<exampleToken>&render=true&url=https%3A%2F%2Fexample.com%2F&window_width=1920'
...
<Tabs>
  <TabItem value="curl" label="Curl">
    <CodeWithEnv>
      ...
      curl --request GET \
        --url '<EnvValue name="dashboardAPI"/>/scraper?api_key=<exampleToken>&render=true&url=https%3A%2F%2Fexample.com%2F&window_height=1080'
...
<Tabs>
  <TabItem value="curl" label="Curl">
    <CodeWithEnv>
      ...
      curl --request GET \
        --url '<EnvValue name="dashboardAPI"/>/scraper?api_key=<exampleToken>&render=true&url=https%3A%2F%2Fexample.com%2F&window_height=1080&window_width=1920'
...
You can use our <EnvLink name="dashboardURL" link="/builder" label="Builder" style={{fontWeight: 700}} /> to generate the instructions easily.
...
<Tabs>
  <TabItem value="curl" label="Curl">
    <CodeWithEnv>
      ...
      curl --request GET \
      --url '<EnvValue name="dashboardAPI"/>/scraper?api_key=<exampleToken>&render=true&url=https%3A%2F%2Fexample.com%2F&instructions=%5B%7B%22wait%22%3A%22.btn%22%7D%2C%7B%22click%22%3A%22.btn2%22%7D%5D'
...
The API will wait up to 30 seconds for the specified selector to appear. If the selector is not found within this time, the request will fail with an error.
...
Yes, enabling JavaScript rendering does increase the cost of the request. It is significantly more resource-intensive, costing approximately ten times more than a standard request. This is due to the need to open a real browser environment to fully render the page.
